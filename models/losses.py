...
class DiceLoss(nn.Module):
    def __init__(self, weight=None, size_average=True, n_classes=2, smooth=1e-5):
        super(DiceLoss, self).__init__()
        self.classes = n_classes
        self.weights = Variable(weight) if weight is not None else None
        self.smooth = smooth

    def forward(self, inputs, target, is_target_variable=False):
        """
        inputs: (N, C, H, W) — model logits
        target: (N, H, W) or (N, 1, H, W) — class indices
        """
        N = inputs.size(0)
        ...
        # Flatten
        inputs_flat = inputs.contiguous().view(N, self.classes, -1)
        target_flat = target_oneHot.contiguous().view(N, self.classes, -1)

        # Compute intersection and union
        intersection = (inputs_flat * target_flat).sum(2)
        union = inputs_flat.sum(2) + target_flat.sum(2)
...
class DiceCELoss(nn.Module):
    def __init__(self, n_classes=2, dice_weight=0.5, ce_weight=0.5, smooth=1e-5, ignore_index=255):
    ...
    def forward(self, inputs, target):
        ...
        # ------------------------
        ce_loss = F.cross_entropy(inputs, target.long(), ignore_index=self.ignore_index)
        ...
        # If 4D label, squeeze to (N, H, W)
        if target.dim() == 4:
            target = torch.squeeze(target, dim=1)

        target_oneHot = to_one_hot_var(target, self.n_classes).float()
        inputs_softmax = F.softmax(inputs, dim=1)

        inputs_flat = inputs_softmax.contiguous().view(N, self.n_classes, -1)
        target_flat = target_oneHot.contiguous().view(N, self.n_classes, -1)
